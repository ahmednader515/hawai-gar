...
## الصور والفيديو (أداء الجوال)

- **قسم الإحصائيات (Stats):** للزوار الذين يبدو أنهم على أجهزة جوّالة (حسب `User-Agent`) أو الذين فعّلوا **Save-Data**، يمكن استخدام **صورة خلفية** بدل ملف MP4 الكبير (`video-2`). **قسم الـ Hero** يعرض دائماً فيديو الخلفية (`video-1`) دون استبداله بصورة.
- **تحسين إضافي:** صوّر PNG كبيرة في `public/` (مثل `land-shipping-1.png`) يمكن ضغطها أو تحويلها إلى WebP خارجياً (مثلاً [Squoosh](https://squoosh.app)) ثم استبدال الملفات.
