          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 lg:gap-12">
            {STATS.map((stat, i) => (
              <div key={i} className="flex flex-col items-center text-center text-white">
                <div className="mb-2 sm:mb-3 opacity-95 w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center">{stat.icon}</div>
                <div className="text-xl sm:text-2xl md:text-3xl font-bold mb-0.5 sm:mb-1 tabular-nums min-h-[1.5em]">{stat.value}</div>
                <div className="text-xs sm:text-sm md:text-base text-white/90 leading-snug">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
